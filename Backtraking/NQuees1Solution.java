...
public class NQuees1Solution {
    ...
    static int  a = 0 ;
    public static boolean nQueensRow(char board[][] , int row){
        ...
        for(int j = 0 ; j<board.length;j++){
            if(issafe(board, row, j) == true){
                 board[row][j] = 'Q';
                 if(nQueensRow(board, row+1)){
                     return true;
                 }
                //recurssion call , if you didnt understand then do the stack analysis , in that the current call will be deletetd abd the next  one will be doen 
                 board[row][j] ='X';
        }
